...
import {
  getAPICallFunction,
  getApiQrCodeclientFunction,
  getApiQrCodeFunction,
  postAPICallFunction,
  QrcodepostAPICallFunction,
...
import {
  ...
  qrclientgetapi,
  qrcodeclientcategoryapi,
  qrclientsubcategoryapi,
  QrPresenturlapi,
  qrTicketretrieveapis,
} from "../../Api/Api";
...
const QrCodeTicketCreate = () => {
  ...
  const createPresntUrlMutation = useMutation({
    ...
    mutationFn: async ({ filepart, responsedatas }) => {
      ...
      const response = await QrcodepostAPICallFunction({
        url: QrPresenturlapi,
  ...
  const { data: Categorydetails, refetch: categoryReCalls } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await getApiQrCodeFunction({
        url: qrcodeclientcategoryapi,
        payload: payloadcategory,
        id: ClientDetailsall.data.id,
      });
      setIsLoading(false);
      return response;
    },
  ...
  }, [Categorydetails, ClientDetailsall]);
  const payloadclient = {
    id: id,
  };
  const { data: Clientdetails, refetch: qrClientdetails } = useQuery({
    queryKey: ["qrClientdetails", id],
    queryFn: async () => {
      setIsLoading(true);

      const response = await getApiQrCodeclientFunction({
        url: qrclientgetapi,
        id: payloadclient.id,
      });
      setIsLoading(false);
...
export default QrCodeTicketCreate;
